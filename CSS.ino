...
void callCSS(WiFiClient client) {               // passed wifi client ?? it works I know not why
  ...
  client.println("body {background-color: DimGrey; color:#e68a00;}");
 ...
 client.println(".marginBox(display: flex; float:left; width: 40px; height: 40px;}");  // could be used for spacing things out
  ...
  client.println(".slider {-webkit-appearance: none;  width: 100%; height: 15px;");   //appearance: none; removed
  ...
  client.println("width: 30px; height: 30px; background: #00cccc;  border-radius: 50%;  cursor: pointer;}");  // Set a specific slider handle width // Slider handle height
  //  Cursor on hover

  client.println(".slider::-moz-range-thumb {width: 40px;  height: 40px;  background: #4CAF50; cursor: pointer;}");
